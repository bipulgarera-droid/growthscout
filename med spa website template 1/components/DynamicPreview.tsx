...
export default function DynamicPreview({ data }: DynamicPreviewProps) {
    const services = (data.theme_settings?.services && data.theme_settings.services.length > 0) ? data.theme_settings.services : (data.services || []);
    const valueProps = (data.value_props && data.value_props.length > 0) ? data.value_props : ["State-of-the-Art Technology", "Experienced Professionals", "Safety & Results Guaranteed"];
    const tagline = data.theme_settings?.tagline || data.tagline || "Experience the difference of expert care and advanced technology.";
    const heroPhrases = (data.theme_settings?.heroPhrases && data.theme_settings.heroPhrases.length > 0) ? data.theme_settings.heroPhrases : (data.hero_phrases || []);
...
function HeroAnimation({ data }: { data: PreviewData }) {
    ...
    const heroPhrases = (data.theme_settings?.heroPhrases && data.theme_settings.heroPhrases.length > 0) ? data.theme_settings.heroPhrases : (data.hero_phrases || []);
    const tagline = data.theme_settings?.tagline || data.tagline || "INNOVATION PERFECTED";
    ...
    const TEXT_OVERLAYS = [
        {
            ...
            startProgress: 0,
            endProgress: 0.25,
            text: heroPhrases[0] || tagline?.toUpperCase() || "INNOVATION PERFECTED",
            highlight: true,
        },
        {
            id: "beat-b",
